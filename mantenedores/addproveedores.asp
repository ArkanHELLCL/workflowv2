...
<%					    
	...
	datos = "'" & PRO_RazonSocial & "'," & PRO_Rut & ",'" & PRO_DV & "','" & PRO_Direccion & "','" & PRO_Telefono	& "','" & PRO_Mail	& "'," & ILD_Id & "," & TCU_Id & ",'" & PRO_NumCuentaBancaria & "'," & PRO_PAC & "," & session("wk2_usrid") & ",'" & session("wk2_usrtoken") & "'"
	...
	sql="exec spProveedores_Agregar " & datos 
	...
	if(not rs.eof) then
		if(rs("Nuevo")=1) then%>
			{"state": 200, "message": "Ejecución exitosa","data": {"PRO_Id" : "<%=rs("PRO_Id")%>", "PRO_RazonSocial" : "<%=rs("PRO_RazonSocial")%>", "PRO_Rut" : "<%=rs("PRO_Rut")%>"}}<%
		else%>
			{"state": 401, "message": "Proveedor ya existe","data": {"PRO_Id" : "<%=rs("PRO_Id")%>", "PRO_RazonSocial" : "<%=rs("PRO_RazonSocial")%>", "PRO_Rut" : "<%=rs("PRO_Rut")%>"}}<%
		end if
	end if
