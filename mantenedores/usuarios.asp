				...
				<div class="view view-cascade gradient-card-header blue-gradient narrower py-2 mx-4 mb-3 d-flex justify-content-between align-items-center" style="height:3rem;">
					<div><%
						if session("wk2_usrperfil")=1 or session("wk2_usrperfil")=2 then%>
							<button class="btn btn-success btn-rounded btn-sm waves-effect" title="Crear un nuevo usuario" type="button" data-url="" data-toggle="tooltip" data-id="10" id="btn_addusuariosModal" name="btn_addusuariosModal">Agregar<i class="fas fa-plus ml-1"></i></button><%
						end if%>
					</div>
					<a href="" class="white-text mx-3"><i class="fas fa-server"></i> Mantenedor de Usuarios</a>
					<div>						
						<button class="btn btn-secondary btn-rounded buttonExport btn-sm waves-effect" data-toggle="tooltip" title="Exportar datos de la tabla" data-id="usuarios">Exportar<i class="fas fa-download ml-1"></i></button>
					</div>
				</div>
				<!--/Card image-->
					...
					<div class="table-wrapper col-sm-12">						
						<!--Table-->
						<table id="tbl-usuarios" class="table-striped table-bordered dataTable table-sm" cellspacing="0" width="100%" data-id="usuarios" >
							<thead>
								<tr> 
									<th>#</th>									
									<th>Perfil</th>
...
<script>
	...
	$(document).ready(function(e) {
		...
		$('#tbl-usuarios tbody').on( 'click', 'td', function (e) {
		...
		function tableusuarios(){			
			...
			usuariosTable = $('#tbl-usuarios').DataTable({
				lengthMenu: [ 10,15,20 ],
				stateSave: true,
				ajax:{
					url:"/tbl-usuarios",					
				...
				"fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {					
					$(nRow).click(function(e){						
						...
						var USR_Id = $(this).find("td")[0].innerText;						
						var data={USR_Id:USR_Id,mode:'mod'}
		...
		$("#btn_addusuariosModal").click(function(e){
			e.preventDefault();
			e.stopImmediatePropagation();
			e.stopPropagation();			
			$.ajax( {
