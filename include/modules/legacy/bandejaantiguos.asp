...
if(session("wk2_usrperfil")=4) and session("wk2_usrjefatura")<>1 then
    response.write("403/@/Perfin no autorizado")
end if
response.write("200/@/")
%>
                                                                ...
                                                                <div class="select">
                                                                    <select name="INFWorkflowv1_Flu" id="INFWorkflowv1_Flu" class="validate select-text form-control" required>
                                                                        ...
                                                                        do while not rs.eof
                                                                            ...
                                                                            <option value="<%=rs("FLU_Id")%>"><%=rs("FLU_Descripcion")%></option><%
                                                                            rs.movenext
                                                                        loop%>
                                                                    </select>							
...
//bandeja antiguos
    ...
    $(document).ready(function() {
		...
		function tableRequerimientos(data){            
            ...
            if(data.SIS_Id==1){
                requerimientosComprasTable=requerimientosTable
            }
            if(data.SIS_Id==2){
                requerimientosWorkFlowv1Table=requerimientosTable
            }
		}        

		$(".content-nav").tabsmaterialize({},function(){
            var SIS_Id = $(this.toString()).data("sis");
			var data={SIS_Id: SIS_Id};

            if(SIS_Id==1){
			    if ( ! $.fn.DataTable.isDataTable( '#tblreq-' + SIS_Id ) ) {
				    tableRequerimientos(data)					
			    }else{
                	requerimientosComprasTable.ajax.reload();
                }                
			}	
		});	
        ...
        $("#btn_frmWorkflowv1").click(function(e){
			...
			if($("#frmwrkflowv1").valid()){
                var Flu = $("#INFWorkflowv1_Flu").val();                
                var Sis = $("#SISWorkflowv1_Id").val();                
                var data = {FLU_Id:Flu, SIS_Id: Sis}
                tableRequerimientos(data)
            }
        })
